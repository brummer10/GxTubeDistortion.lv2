...
#define GTK_TUBEDISTORTION_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_TUBEDISTORTION_KNOB, GtkTubeDistortionKnobPrivate))

G_DEFINE_TYPE(GtkTubeDistortionKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkTubeDistortionKnobPrivate GtkTubeDistortionKnobPrivate;

struct _GtkTubeDistortionKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkTubeDistortionKnobPrivate *priv = GTK_TUBEDISTORTION_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkTubeDistortionKnob *knob = GTK_TUBEDISTORTION_KNOB(widget);
...
{
    g_assert(GTK_IS_TUBEDISTORTION_KNOB(widget));
    GtkTubeDistortionKnob *knob = GTK_TUBEDISTORTION_KNOB(widget);
    GtkTubeDistortionKnobPrivate *priv = GTK_TUBEDISTORTION_KNOB_GET_PRIVATE(knob);
...
{
    GtkTubeDistortionKnob *knob = GTK_TUBEDISTORTION_KNOB(object);
...
/****************************************************************
 ** init the GtkTubeDistortionKnobClass
 */

static void gtk_knob_class_init (GtkTubeDistortionKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkTubeDistortionKnobPrivate));
}
...
static void gtk_knob_init (GtkTubeDistortionKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_TUBEDISTORTION_KNOB, NULL ));
