...
static void resize_event(gx_TubeDistortionUI *ui);
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	ui->controls[0] = { 1.0, 1.0, 0.0, 1.0, 1.0, 30, 40, 61, 61, false,"POWER", BSWITCH, BYPASS};
	ui->controls[1] = { 0.5, 0.5, 0.0, 1.0, 0.01, 120, 40, 61, 61, false,"INPUT", KNOB, INPUT};
	ui->controls[2] = { 0.5, 0.5, 0.0, 1.0, 0.01, 210, 40, 61, 61, false,"OUTPUT", KNOB, OUTPUT};
...
static void _expose(gx_TubeDistortionUI *ui) {
...
static void controller_expose(gx_TubeDistortionUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static bool get_active_ctl_num(gx_TubeDistortionUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_TubeDistortionUI *ui, int direction) {
	float value;
	int num;
...
static void button1_event(gx_TubeDistortionUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_TubeDistortionUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_TubeDistortionUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_TubeDistortionUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_TubeDistortionUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_TubeDistortionUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void event_handler(gx_TubeDistortionUI *ui) {
	XEvent xev;
